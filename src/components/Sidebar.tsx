...
export const Sidebar = ({isOpen}: {isOpen: boolean}) => {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 z-50 h-screen bg-boxdark p-6 pt-9 overflow-x-hidden transition-all duration-300 ease-linear ${
        isOpen ? 'w-72' : 'w-20'
      }`}>
      <Link to='/' className='flex items-center gap-2 transition-all duration-300 ease-in-out'>
        <img src={logo} alt='logo' />
        <h1 className={`text-3xl text-white font-medium duration-200 ${!isOpen && 'scale-0'}`}>
          TailAdmin
        </h1>
      </Link>
      <nav>
        <ul className='w-full text-bodydark1 pt-16'>
          {navItems.map(({ title, src, icon }) => (
            <li key={title} className='h-11 hover:bg-darkhovered rounded-md mb-2 duration-200'>
              <NavLink
                to={src}
                className={navData =>
                  `w-full h-full flex items-center gap-3 px-2 rounded-md ${
